...
let keyword = "";
let url=`https://api.themoviedb.org/3/discover/movie?api_key=${api}&language=en-US&sort_by=popularity.desc&page=1`

function Render() {
  let [movieInfo, setmovieInfo] = useState(null);
  let [modal, setModal] = useState(false);
  let [key, setKey] = useState("");
  let [mainPageNumber, setMainPageNumber] = useState(1);
  let [sortType, setSortType] = useState("popularity");
  let [sortBy, setSortBy] = useState("desc");
  // let [url, setUrl] = useState(`https://api.themoviedb.org/3/discover/movie?api_key=${api}&language=en-US&sort_by=${sortType}.${sortBy}&page=${mainPageNumber}`);
  ...
  let changePage = (newPageNum) => {
    ...
    setMainPageNumber(newPageNum);
    url = `https://api.themoviedb.org/3/discover/movie?api_key=${api}&language=en-US&sort_by=${sortType}.${sortBy}&page=${newPageNum}`;
    fetchMovieInfo(url);
    console.log(url);
  };
  ...
  let changeSortType = (type) => {
    setSortType(type);
    url = `https://api.themoviedb.org/3/discover/movie?api_key=${api}&language=en-US&sort_by=${type}.${sortBy}&page=${mainPageNumber}`;
    fetchMovieInfo(sortType);
  };

  let changeSortBy = (type) => {
    setSortBy(type);
    url = `https://api.themoviedb.org/3/discover/movie?api_key=${api}&language=en-US&sort_by=${sortType}.${type}&page=${mainPageNumber}`;
    fetchMovieInfo(sortBy);
  };

  let search = async (keyword) => {
    let url = `https://api.themoviedb.org/3/search/movie?query=${keyword}?api_key=${api}&language=en-US`;
    let data = await fetch(url);
    let result = await data.json();
    setmovieInfo(result);
  };
